...
class Texture {
   private:
    inline static const std::filesystem::path DEFAULT_TEXTURE1 = System::DEFAULT_TEXTURE_PATH / "texture1.png";
    inline static const std::filesystem::path DEFAULT_TEXTURE2 = System::DEFAULT_TEXTURE_PATH / "texture2.png";
   ...
   public:
    ...
    Texture(
        const std::filesystem::path& path = DEFAULT_TEXTURE1,
    ...
    friend struct fmt::formatter<Texture>;
};

}  // namespace Renderer

template <> struct fmt::formatter<Renderer::Texture> : fmt::formatter<std::string_view> {
    template <typename FormatContext>
    auto format(const Renderer::Texture& tex, FormatContext& ctx) -> decltype(ctx.out()) {
        return format_to(
            ctx.out(),
            "\nTexture ID: {}\n\tTexture Unit (offset): {}\n\tTexture Path: {}",
            tex.texId,
            static_cast<int>(tex.texUnit) - static_cast<int>(TexUnit::T0),
            tex.path
        );
    }
};
